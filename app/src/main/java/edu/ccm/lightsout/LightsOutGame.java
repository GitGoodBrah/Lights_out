...
public class LightsOutGame {
    ...
    public void newGame() {
        ...
        for (int row = 0; row < GRID_SIZE; row++)
            for (int col = 0; col <GRID_SIZE; col++)
                mLightsGrid[row][col] = random.nextBoolean();
    }
    ...
    public void selectLight(int row, int col){
        ...
        if (row > 0) // flip the light above the selected one
            mLightsGrid[row-1][col] = !mLightsGrid[row-1][col];
        if (row < GRID_SIZE -1) //flip the light below
            mLightsGrid[row+1][col] = !mLightsGrid[row+1][col];
        if (col > 0) // flip to the left
            mLightsGrid[row][col-1] = !mLightsGrid[row][col-1];
        if (col < GRID_SIZE -1) // flip to the right
            mLightsGrid[row][col+1] = mLightsGrid[row][col+1];
